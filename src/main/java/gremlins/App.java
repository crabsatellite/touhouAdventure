...
public class App extends PApplet {
    ...
    */
    ...
    private void doLoadImage() {
        // Load static objects
        imageMap.put('X', loadImage(Objects.requireNonNull(this.getClass().getResource("stonewall.png")).getPath().replace("%20", " ")));
        imageMap.put('B', loadImage(Objects.requireNonNull(this.getClass().getResource("brickwall.png")).getPath().replace("%20", " ")));
        imageMap.put('F', loadImage(Objects.requireNonNull(this.getClass().getResource("fireball.png")).getPath().replace("%20", " ")));
        imageMap.put('G', loadImage(Objects.requireNonNull(this.getClass().getResource("gremlin.png")).getPath().replace("%20", " ")));
        imageMap.put('S', loadImage(Objects.requireNonNull(this.getClass().getResource("slime.png")).getPath().replace("%20", " ")));
        imageMap.put('E', loadImage(Objects.requireNonNull(this.getClass().getResource("exit.png")).getPath().replace("%20", " ")));
        imageMap.put('P', loadImage(Objects.requireNonNull(this.getClass().getResource("powerup.png")).getPath().replace("%20", " ")));
        imageMap.put('U', loadImage(Objects.requireNonNull(this.getClass().getResource("supergremlin.png")).getPath().replace("%20", " ")));
        ...
        for (String dynamicObjectName : dynamicObjectNameArray) {
            ...
            for (int j = 0; j < 4; j++) { // wizard and brickwall_destroyed all have 4 states. j < 4
                imageList.add(loadImage((Objects.requireNonNull(this.getClass().getResource(dynamicObjectName + j + ".png")).getPath().replace("%20", " "))));
            }
        ...
        for (int i = 0; i < gameStageArray.size(); i++) {
            ...
            try {
                ...
                if (isValidateMap(mapDataMatrix)){
                    gameMapList.add(new GameMap(gameMapLayout, wizardAttackCooldown, enemyAttackCooldown, mapDataMatrix));//can design by myself?
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private boolean isValidateMap (char[][] mapDataMatrix) throws Exception {
        ...
        if (mapDataMatrix != null && dummyWizardPosition != null) {
            ...
            boolean isValidateMap = checkMapValidity(tempTestMap, dummyWizardPosition[0], dummyWizardPosition[1]) && mapHasExit && mapHasWizard;
    ...
    private boolean checkMapValidity(char[][] map, int row, int col) {
        if (row < 0 || col < 0 || row > MAP_ROW - 1 || col > MAP_COLUMN - 1) return false; // corner cases
