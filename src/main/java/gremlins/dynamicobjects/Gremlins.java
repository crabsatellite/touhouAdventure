...
public class Gremlins extends Animation {
  ...
  public void gremlinsMove() {
    ...
    if (gremlinsDirection == App.UP
        && entityCanWalk(wallList, gremlinsX - GREMLINS_MOVE_SPEED, gremlinsY)) {
      super.setAnimationObjectPosX(gremlinsX - GREMLINS_MOVE_SPEED);
      super.setDirection(App.UP);
    } else if (gremlinsDirection == App.LEFT
        && entityCanWalk(wallList, gremlinsX, gremlinsY - GREMLINS_MOVE_SPEED)) {
      super.setAnimationObjectPosY(gremlinsY - GREMLINS_MOVE_SPEED);
      super.setDirection(App.LEFT);
    } else if (gremlinsDirection == App.DOWN
        && entityCanWalk(wallList, gremlinsX + GREMLINS_MOVE_SPEED, gremlinsY)) {
      super.setAnimationObjectPosX(gremlinsX + GREMLINS_MOVE_SPEED);
      super.setDirection(App.DOWN);
    } else if (gremlinsDirection == App.RIGHT
        && entityCanWalk(wallList, gremlinsX, gremlinsY + GREMLINS_MOVE_SPEED)) {
      super.setAnimationObjectPosY(gremlinsY + GREMLINS_MOVE_SPEED);
  ...
  public void slimeVapourised(List<Fireball> fireballList) {
    ...
    while (fireballIterator.hasNext()) {
      ...
      while (slimeIterator.hasNext()) {
        ...
        if ((slimeX >= fireballX - SPRITESIZE / ANTI_COLLISION_CONST)
            && (slimeX <= (fireballX + SPRITESIZE / ANTI_COLLISION_CONST))
            && (slimeY >= fireballY - SPRITESIZE / ANTI_COLLISION_CONST)
            && slimeY <= (fireballY + SPRITESIZE / ANTI_COLLISION_CONST)) {
