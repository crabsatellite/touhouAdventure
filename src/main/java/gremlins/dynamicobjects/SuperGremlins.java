...
public class SuperGremlins extends Animation {
    ...
    public SuperGremlins(int x, int y, GameMap gameMap) {
        ...
        this.superGremlinsAttackCooldown = (int) gameMap.getEnemyAttackCooldown() * FPS / 2; // Double the rate of fire compared to gremlins
    }
    ...
    public void superGremlinsMove() {
        ...
        if (superGremlinsDirection == App.UP && entityCanWalk(wallList, superGremlinsX - SUPER_GREMLINS_MOVE_SPEED, superGremlinsY)) {
            super.setAnimationObjectPosX(superGremlinsX - SUPER_GREMLINS_MOVE_SPEED);
            super.setDirection(App.UP);
        } else if (superGremlinsDirection == App.LEFT && entityCanWalk(wallList, superGremlinsX, superGremlinsY - SUPER_GREMLINS_MOVE_SPEED)) {
            super.setAnimationObjectPosY(superGremlinsY - SUPER_GREMLINS_MOVE_SPEED);
            super.setDirection(App.LEFT);
        } else if (superGremlinsDirection == App.DOWN && entityCanWalk(wallList, superGremlinsX + SUPER_GREMLINS_MOVE_SPEED, superGremlinsY)) {
            super.setAnimationObjectPosX(superGremlinsX + SUPER_GREMLINS_MOVE_SPEED);
            super.setDirection(App.DOWN);
        } else if (superGremlinsDirection == App.RIGHT && entityCanWalk(wallList, superGremlinsX, superGremlinsY + SUPER_GREMLINS_MOVE_SPEED)) {
            super.setAnimationObjectPosY(superGremlinsY + SUPER_GREMLINS_MOVE_SPEED);
...
//            superGremlinsAttackCooldown = (int) getGameMap().getEnemyAttackCooldown() * FPS; // Double the attack rate
//            superSlimeList.add(new SuperSlime(superGremlinsX, superGremlinsY, getGameMap(), superGremlinsDirection));
//        } else {
