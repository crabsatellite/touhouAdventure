...
public class GameSystem {
  ...
  public GameSystem(
      PApplet pApplet,
      Map<Character, PImage> imageMap,
      Map<Character, List<PImage>> animationMap,
      List<GameMap> gameMapList,
      int wizardLives) { // unfinished
  ...
  public void refreshFrame() { // draw new images per frame
  ...
  private void generatePowerUpItem() {
    ...
    while (powerUpRow < 0
        || powerUpCol < 0
        || powerUpCol >= MAP_COLUMN
        || powerUpRow >= MAP_ROW
        || gameMap.getMapDataMatrix()[powerUpRow][powerUpCol] != ' ') {
      powerUpRow = randomGenerator.nextInt() % MAP_ROW;
      powerUpCol = randomGenerator.nextInt() % MAP_COLUMN;
    }
  }
  ...
  private void drawInformationBar() {
    ...
    pApplet.fill(0, 0, 0);
    pApplet.text(
        "level " + (stage + 1) + "/" + gameMapList.size(), 10 * SPRITESIZE, 34 * SPRITESIZE + 8);

    if (buffTime != 0) {
      pApplet.textSize(20);
      pApplet.fill(0, 0, 0);
      pApplet.text(
          "POWER UP TIME REMAIN: " + buffTime / 60 + "s", 10 * SPRITESIZE, 34 * SPRITESIZE + 30);
    }
    ...
    if (gameMap.getWizard().getWizardAttackCooldown() != 0) {
      ...
      if (gameMapCooldown != 0) {
        ...
        int black =
            (int) (gameMap.getWizard().getWizardAttackCooldown() / gameMapCooldown * length);
        pApplet.fill(0, 0, 0);
  ...
  private void checkIfCharacterDead() {
    ...
    while (superGremlinsIterator.hasNext()) {
      ...
      if (superGremlins.hitByFireball(fireballList)) {
        superGremlins.superGremlinsRespawn(
            wizard.getAnimationObjectPosX(), wizard.getAnimationObjectPosY());
      }
  ...
  private void drawStaticItems() {
    ...
    for (StaticObjects solidWall : solidWallListList) {
      pApplet.image(
          imageMap.get('X'), solidWall.getCol() * SPRITESIZE, solidWall.getRow() * SPRITESIZE);
    }
    ...
    while (brickWallIterator.hasNext()) {
      ...
      if (brickWallState == -1) {
        pApplet.image(
            imageMap.get('B'), brickWall.getCol() * SPRITESIZE, brickWall.getRow() * SPRITESIZE);
      } else {
        if (brickWallState < 4) {
          pApplet.image(
              animationMap.get('B').get(brickWallState),
              brickWall.getCol() * SPRITESIZE,
              brickWall.getRow() * SPRITESIZE); // unfinished?
        } else {
  ...
  private void drawAnimation() {
    ...
    while (superGremlinsIterator.hasNext()) {
      ...
      pApplet.image(
          imageMap.get('U'),
          superGremlins.getAnimationObjectPosY(),
          superGremlins.getAnimationObjectPosX());
      if (buffTime == 0) {
    ...
    while (gremlinsIterator.hasNext()) {
      Gremlins gremlins = gremlinsIterator.next();
      pApplet.image(
          imageMap.get('G'), gremlins.getAnimationObjectPosY(), gremlins.getAnimationObjectPosX());
      if (buffTime == 0) {
